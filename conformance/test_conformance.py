...
wc/wcl comparison as an individual pytest case for nicer local reporting and
...
CI entrypoint; this is for ergonomics.
    ...
    WCL_CONF_FUZZ=500 pytest conformance/    # deeper
    pytest conformance/ -k "UTF8 and chars"  # slice the matrix

A prebuilt wcl is required (cmake --build <dir> --target wcl) or set $WCL_BIN.
"""
...
# pytest materializes one test object per comparison; bump WCL_CONF_FUZZ for more.
_SESSION = build_session()
_SEED = int(os.environ.get("WCL_CONF_SEED", "1234"))
_FUZZ = int(os.environ.get("WCL_CONF_FUZZ", "60"))

_TMP = tempfile.mkdtemp(prefix="wcl_conf_pytest_")
...
def test_session_has_oracle():
    ...
    assert os.path.isfile(_SESSION.wcl_bin)
